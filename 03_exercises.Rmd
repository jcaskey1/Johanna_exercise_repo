...
```{r}
garden_harvest_sum <- garden_harvest %>%
  mutate(day = wday(date, label = TRUE)) %>%
  group_by(vegetable, day) %>%
  mutate(total_harvest_lbs = sum(weight) * 0.00220462) %>%
...
```{r}
garden_harvest_plot <- garden_harvest %>% 
  group_by(vegetable, variety) %>%
  summarise(total_harvest_lbs = sum(weight) * 0.00220462) %>%
  left_join(select(plant_date_loc,vegetable, variety, plot), by = c("vegetable", "variety"))
```
...
ggplot(tomatoes, aes(x = reorder(str_to_title(variety), first_harvest), y = sum(weight) * 0.00220462)) +
  geom_bar(stat = "identity", fill = "#ef3b2c") + 
  ggthemes::theme_few() +
...
```{r}
garden_harvest_str <- garden_harvest %>%
  group_by(vegetable, variety) %>%
  mutate(var_lowercase = str_to_lower(variety),
         var_name_len = str_length(variety)) %>%
  group_by(vegetable, var_lowercase) %>%
  distinct(var_name_len) %>%
...
```{r}
garden_harvest_distinct <- garden_harvest %>%
...
Trips %>%
  ggplot(aes(x = sdate)) +
  geom_density(fill = "#9cb8e6") +
 labs(x = "Start Date", y = "Events", title = "Distribution of Events by Start Date") +
 ggthemes::theme_few()
```
...
Trips %>%
  ...
  ggplot +
 aes(y = weekday, fill = weekday) +
 geom_bar() +
 scale_fill_brewer(palette = "Spectral") +
   labs(x = "Count", y = "Weekday", title = "Events on Each Weekday") +
 ggthemes::theme_few() +
...
Trips %>% 
 ...
 labs(x = "Time of Day", y = "Events", title = "Distribution of Events Each Weekday by Time of Day") +
...
Trips %>% 
  ...
  mutate(weekday = wday(sdate, label = TRUE)) %>%
  ggplot() +
 aes(x = time, fill = client) +
 geom_density(color = NA, alpha = 0.5) +
 scale_fill_brewer(palette = "Accent") +
 labs(x = "Time", y = "Events ", title = "Events by Time of Day Each Weekday", subtitle = "Based on Client Type") +
 ggthemes::theme_few() +
 facet_wrap(vars(weekday))
```
...
```{r}
Trips %>% mutate(hour = hour(sdate),
                 minute = minute(sdate)) %>% 
  mutate(time = hour + (minute/60)) %>% 
  mutate(weekday = wday(sdate, label = TRUE)) %>%
  mutate(is_weekend = ifelse(weekday == c("Sat", "Sun"), "Weekend", "Weekday")) %>%
    ggplot() +
 aes(x = time, fill = client) +
 geom_density(color = NA, alpha = 0.5) +
 scale_fill_brewer(palette = "Pastel2") +
 labs(x = "Time", y = "Events ", title = "Events by Time of Day on Weekdays vs Weekends", subtitle = "Based on Client Type") +
 ggthemes::theme_few() +
...
ggplot(sta_trip) +
 aes(x = lat, y = long, colour = sstation) +
 geom_tile(size = 1L) +
 scale_color_hue() +
 theme_minimal() +
  theme(legend.position = "none") 
...
Trips %>%
  ...
  ggplot() +
 aes(x = lat, y = long, colour = percent_cas) +
 geom_point(size = 2L) +
 scale_color_distiller(palette = "RdYlBu") +
 labs(x = "Latitude", y = "Longitude", title = "Percentage of Casual Bike Renters by Location") +
...
```{r}
top_10 <- Trips %>%
  mutate(dep_date = as_date(sdate)) %>%
  group_by(sstation, dep_date) %>%
  mutate(count = n()) %>%
  arrange(desc(count)) %>%
  distinct(sstation, dep_date, count) %>%
  head(10)
...
```{r}
top_10_full <- Trips %>% inner_join(top_10, by = "sstation", "sdate" = "dep_date")
...
```{r}
top_10_prop <- Trips %>% 
  inner_join(top_10, by = "sstation", "sdate" = "dep_date") %>%
  mutate(weekday = wday(dep_date, label = TRUE)) %>%
  group_by(client, weekday) %>%
  summarise(cli_num = n()) %>%
  mutate(prop = cli_num/sum(cli_num)) %>%
